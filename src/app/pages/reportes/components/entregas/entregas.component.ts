...
export class EntregasComponent implements OnInit {
  ...
  cargarDataInicial() {
    ...
    this.reporteService.getRemitoPorMes(this.reporteFechas).subscribe(dataset => {
  ...
  filtrarFechas() {
    ...
    this.reporteService.getRemitoPorMes(this.reporteFechas).subscribe(dataset => {
      this.chart.destroy();
      if (dataset.length === 0) {
        this.openSnackBar("No se registraron entregas en el periodo de fecha CARGADOS")
      }
      this.createChart(dataset);
    });
  }
  ...
  createChart(dataset: any[]): void {
    ...
    this.chart = new Chart('remito', config);
  }
